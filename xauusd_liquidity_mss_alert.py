...
Stage 2: On subsequent 1m candle closes, checks whether price closes
         ...
         with a full trade card:
           - Entry: the confirming 1m close
           - Stop Loss: beyond the sweep candle's wick + 0.05% buffer
           - Take Profit: shown BOTH as a fixed 1:2 R:R target AND as
             the opposing HTF level (e.g. swept Day High -> Day Low)
...
def default_state():
    return {
        ...
        #   "level": float,          # swing point that must be closed beyond for MSS
        #   "sweep_extreme": float,  # the wick price that took liquidity (for SL)
        #   "swept_period": str,     # e.g. "day" -- used to fetch the opposing HTF level for TP
        # }
    }
...
def main():
    ...
    if swept_this_run:
        ...
        if sell_sweeps:
            ...
            # largest/most significant period swept this run (list is in month->4h
            # order) -> its opposing level is used as the HTF target later
            widest_period = sell_sweeps[0][1]
            if swing_low is not None:
                state["mss_pending"] = {
                    "direction": "bearish",
                    "level": swing_low,
                    "sweep_extreme": last_closed["high"],
    ...
    if pending:
        ...
        if pending["direction"] == "bearish" and last_closed["close"] < level:
            ...
            msg = (
                f"✅ BEARISH MSS CONFIRMED — SHORT SETUP\n"
                f"Entry: {entry:.2f}\n"
            ...
            msg = (
                f"✅ BULLISH MSS CONFIRMED — LONG SETUP\n"
                f"Entry: {entry:.2f}\n"
                f"Stop Loss: {sl:.2f}  (risk {risk:.2f})\n"
                f"TP (1:{RR_MULTIPLE} R:R): {tp_rr:.2f}\n"
            )
